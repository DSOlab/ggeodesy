...
int main() {
  ...
  do {
    ...
    printf("\n |φ|    |%+04i %02i %10.7f|  |φ|    |%+04i %02i %10.7f|", a1_deg,
           a1_min, a1_sec, b1_deg, b1_min, b1_sec);
    printf("\n |λ|  = |%+04i %02i %10.7f|  |λ|  = |%+04i %02i %10.7f|", a2_deg,
           a2_min, a2_sec, b2_deg, b2_min, b2_sec);
    ...
    printf("\nForward Azimouth           : %+3d %2d %8.5f", a_deg, a_min,
           a_sec);
    rad2hexd(a_bac, a_deg, a_min, a_sec, sign);
    printf("\nBackward Azimouth          : %+3d %2d %8.5f", a_deg, a_min,
           a_sec);
    printf("\nBy the way, the Haversine formula computes the distance to be: "
           "%20.3f",
           S_haver);
    printf("\nCompared to Vincenty's algorithm, the bias is %15.5f",
           std::abs(S - S_haver));
  ...
  printf("\nfrom the above computation and perform the direct Vincenty
  Formula"); printf("\nto check results.");
  ...
  printf("\nLatitude   : %+15.5f sec.",
  rad2deg(std::abs(new_lat-deg2rad(p2.x)))*3600.0); printf("\nLongtitude :
  %+15.5f sec.", rad2deg(std::abs(new_lon-deg2rad(p2.y)))*3600.0);
  ...
  printf("\nInternal accuracy of the Inverse vs Direct Vincenty
