...
int main()
{
  ...
  for (int i=0; i<150; ++i) {
    ...
    assert(std::abs(ngpt::hexd2decd(deg, min, seconds)-angle_degrees)<ErrLimit);
    ...
    assert(std::abs(ngpt::hexd2rad(deg, min, seconds)-angle_radians)<ErrLimit);
    ...
    theta = ngpt::normalize_angle(angle_radians, -ngpt::DPI, ngpt::DPI);// [-π, π)
    assert(std::abs(std::sin(angle_radians)-std::sin(theta))<ErrLimit);
    assert(std::abs(std::cos(angle_radians)-std::cos(theta))<ErrLimit);
  }
