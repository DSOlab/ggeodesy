...
#ifdef CHECK_PRECISION
    if ((dlat=std::abs(m_rad2meters<ellipsoid::wgs84>(lat1-lat2,lat1)))>max_dlat)
      max_dlat = dlat;
    if ((dlon=std::abs(p_rad2meters<ellipsoid::wgs84>(lon1-lon2,lat1)))>max_dlon)
      max_dlon = dlon;
    if ((dhgt=std::abs(hgt1-hgt2))>max_dhgt)
      max_dhgt = dhgt;
#else
    assert(approxEqual(lat1,lat2) && approxEqual(lon1,lon2) && approxEqual(hgt1,hgt2));
#endif
