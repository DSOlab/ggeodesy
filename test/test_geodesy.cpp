...
int main() {
  ...
  printf("\ndx=%8.5f dy=%8.5F dz=%8.5f\n", std::abs(p1.x - p3.x),
         std::abs(p1.y - p3.y), std::abs(p1.z - p3.z));
  assert((std::abs(p1.x - p3.x) < 1e-7) && (std::abs(p1.y - p3.y) < 1e-7) &&
         (std::abs(p1.z - p3.z) < 1e-7));

  /// compute the topocentric vector between p1 and p3 ...
  car2top<ellipsoid::grs80>(p1.x, p1.y, p1.z, p3.x, p3.y, p3.z, p2.x, p2.y,
                            p2.z);
  /// ... and print the components (should be zero)
  printf("Topocentric vector (should be zero!)");
  printf("\ndn=%8.5f de=%8.5f du=%8.5f\n", p2.x, p2.y, p2.z);
  assert((std::abs(p2.x) < 1e-7) && (std::abs(p2.y) < 1e-7) &&
         (std::abs(p2.z) < 1e-7));
  ...
  double dx = p1.x - p3.x, dy = p1.y - p3.y, dz = p1.z - p3.z;
  ...
  // assert( std::abs(a_for-hexd2rad(306, 52, 5.37)) < 3*1e-5 ); // .1 sec
  // accuracy assert( std::abs(a_bac-hexd2rad(127, 10,25.07)) < 3*1e-5 ); // .1
  // sec accuracy
  ...
  printf("Going back the geodetic to point 2, yields differences: (i.e. in "
         "point 2)\n");
