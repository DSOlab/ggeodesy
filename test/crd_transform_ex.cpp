...
int main ()
{
    ...
    printf("\n\nCartesian and Topocentric vectors, between Pa and Pb (i.e Pb-Pa) are:");
    ...
    car2top<ellipsoid::grs80>(p1.x, p1.y, p1.z, p2.x, p2.y, p2.z, p4.x, p4.y, p4.z);
    //  However, since we already have the [dx,dy,dz] vector, we could transform
    //+ it directly to topocentric; i.e. instead of using:
    //+ car2top<ellipsoid::grs80>(p1.x, p1.y, p1.z, p2.x, p2.y, p2.z, p4.x, p4.y, p4.z);
    //+ we could have used:
    Point p4_2;
    dcar2top<ellipsoid::grs80>(p1.x, p1.y, p1.z, p3.x, p3.y, p3.z, p4_2.x, p4_2.y, p4_2.z);
    // Now verify that both functions give the same results
    assert( std::abs(p4_2.x-p4.x) < prec
         && std::abs(p4_2.y-p4.y) < prec
    ...
    for (p = max_prec; p >= min_prec; p *= 1e-1) {
        if (! (std::abs(p4_2.x-p4.x) < prec               
          && std::abs(p4_2.y-p4.y) < prec
          && std::abs(p4_2.z-p4.z) < prec) ) break;
    }
    printf("\nInternal precision of the two car2top implementations, is approximately %e meters", p);
    ...
    //  From the local, topocentric vector p3 = [north, east, up], we can calculate
    ...
    //  Check to what precision we get the same results
    for (p = max_prec; p >= min_prec; p *= 1e-1) {
        if (! (std::abs(p6.x-p6.x) < prec               
          && std::abs(p6.y-p3.y) < prec
          && std::abs(p6.z-p6.z) < prec) ) break;
    }
