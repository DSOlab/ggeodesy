...
auto test_ggeodesy_vincenty(const std::vector<TestLine> &vec, double *maxer,
                            double *meaner) {
...
auto test_ggeodesy_karney(const std::vector<TestLine> &vec, double *maxer,
                          double *meaner) {
  double maxer_lat = std::numeric_limits<double>::min(),
         maxer_lon = std::numeric_limits<double>::min(),
         maxer_raz = std::numeric_limits<double>::min();
...
auto test_boost_vincenty(const std::vector<TestLine> &vec, double *maxer,
                         double *meaner) {
  double maxer_lat = std::numeric_limits<double>::min(),
         maxer_lon = std::numeric_limits<double>::min(),
         maxer_raz = std::numeric_limits<double>::min();
  ...
  for (const auto &dataset : vec) {
    result_drc = vincenty_direct_type::apply(
        dataset.lon1(), dataset.lat1(), dataset.distance(),
        dataset.forward_azimuth(), spheroid);
    ...
    if ((err_az = std::abs(dataset.backward_azimuth() -
                           result_drc.reverse_azimuth)) > maxer_raz) {
      maxer_raz = err_az;
    }
    meaner_lat += err_lat;
    meaner_lon += err_lon;
...
auto test_boost_karney(const std::vector<TestLine> &cvec, double *maxer,
                       double *meaner) {
  double maxer_lat = std::numeric_limits<double>::min(),
         maxer_lon = std::numeric_limits<double>::min(),
         maxer_raz = std::numeric_limits<double>::min();
  ...
  for (auto &dataset : vec) {
    for (int i = 0; i < 6; i++)
      dataset.ar[i] = rad2deg(dataset.ar[i]);
  }
  auto start = high_resolution_clock::now();
  for (const auto &dataset : vec) {
    result_drc = karney_direct_type::apply(dataset.lon1(), dataset.lat1(),
                                           dataset.distance(),
                                           dataset.forward_azimuth(), spheroid);
    if ((err_lat = std::abs(dataset.lat2() - result_drc.lat2)) > maxer_lat) {
      maxer_lat = err_lat;
...
int main() {
  ...
  for (const auto &batch : GeodTest_descr) {
    ...
    printf("\n%50s %17.15f %17.15f %17.15f Max Err.", batch.second.c_str(),
           rad2sec(maxer[0]), rad2sec(maxer[1]), rad2sec(maxer[2]));
    printf("\n%50s %17.15f %17.15f %17.15f Mean Err. (time: %10ld microsec)",
           batch.second.c_str(), rad2sec(meaner[0]), rad2sec(meaner[1]),
           rad2sec(meaner[2]), duration.count());
